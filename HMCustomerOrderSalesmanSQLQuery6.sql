...
insert into Salesman1 values('Nitesh','Mumbai',300)
insert into Salesman1 values('Riyansh','Nashik',100)
insert into Salesman1 values('Rajshri','Mumbai',200)
insert into Salesman1 values('Priti','Pune',500)
insert into Salesman1 values('Gitesh','Mumbai',100)
insert into Salesman1 values('Vedant','Pune',1000)
...
--Add Foreign key in Order1 table 
alter table Orders1
add foreign key (saleman_id) references Salesman1(saleman_id)
...
select c.c_id,c.cust_name,c.grade
from Cust c
right join Salesman1 s on  s.saleman_id=c.c_id
where c.c_id=3

--6.full Join   return all record from right table and matches data from left table
select c.c_id,c.cust_name,c.grade
from Cust c
full join Salesman1 s on  s.saleman_id=c.c_id
where c.c_id=6
...
from Cust c
inner join Salesman1 s on c.c_id=s.saleman_id
order by c.c_id
